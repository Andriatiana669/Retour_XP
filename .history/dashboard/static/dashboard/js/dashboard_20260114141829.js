document.addEventListener('DOMContentLoaded', () => {
    ...
    // construit un texte « plat » pour un objet quelconque
    const flatten = obj => JSON.stringify(obj).replace(/"|{|}/g, ' ');

    // surligne (wrap <mark>) les occurrences de needle dans haystack
    ...
    searchInput.addEventListener('input', () => {
        ...
        marcheCards.forEach(card => {
            ...
            // on va aussi alimenter la zone détails si needle est présent
            let detailHtml = '';
        ...
        // affiche/masque la zone détaillée
        detailsZone.classList.toggle('d-none', !hasResults);
    });

    // animation clic (déjà présente)

    ...
    searchInput.addEventListener('input', () => {
        ...
        marcheCards.forEach(card => {
            ...
            detailArr.forEach(md => {
                /* on ajoute nom_marche dans le texte de recherche */
                ...
                if (mdMatch || carteHtml) {
                    hasResults = true;
                    /* on affiche aussi nom_marche s'il existe */
                    const nomMarcheTag = md.nom_marche
                        ? `<div class="mb-1"><strong>Nom marché :</strong> ${highlight(md.nom_marche, needle)}</div>`
                        : '';
                    detailHtml += `

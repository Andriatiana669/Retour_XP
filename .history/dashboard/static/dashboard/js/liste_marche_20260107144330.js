...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $$('thead th').forEach((th, idx) => {
        if (idx > 2) return; // on ne tri que les 3 premières colonnes
        th.style.cursor = 'pointer';
        th.insertAdjacentHTML('afterbegin', '<i class="fas fa-sort ms-1 text-muted"></i>');
    ...
    $('.center-buttons').before(dateFilter);
    ...
    $('.center-buttons').before(etatFilter);
    ...
    $('.center-buttons').before(respFilter);
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', async () => {
            ...
            for (const tr of rows) {
                if (tr.style.display === 'none') continue; // on ignore les lignes cachées
                const id = tr.dataset.id;
                const etatId = tr.querySelector('select[name="etat"]').value;
                try {

...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ---------- SAUVEGARDE INDIVIDUELLE (bouton Actualiser) ---------- */
    document.querySelectorAll('.save-etat-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (resp.ok) {
                    /* --- MISE À JOUR LOCALES --- */
                    const data = await resp.json();          // si tu renvoies la nouvelle date
                    tr.cells[0].textContent = data.date_maj; // ou fais un petit toast
                    select.dataset.initial = etatId;         // mémorise la valeur
                    btn.style.color = '#28a745';             // feedback visuel

...
def dashboard_view(request):
    ...
    if q:
        ...
        md_qs = md_qs.filter(
            Q(titre__icontains=q) | Q(objet__icontains=q) | Q(description__icontains=q) | Q(matricule__icontains=q)
        )
        c_qs = c_qs.filter(
            Q(titre__icontains=q) | Q(objet__icontains=q) | Q(contenu__icontains=q) | Q(nom_client__nom__icontains=q)
        )
    ...
    if type_filter == 'global':
        ...
        # on concatène en Python
        qs = list(mg_qs) + list(md_qs) + list(c_qs)

    # On ajoute les propriétés complémentaires
    results = []
    for obj in qs:

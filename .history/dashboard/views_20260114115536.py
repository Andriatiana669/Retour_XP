...
def dashboard_view(request):
    ...
    # 1) Grille d’origine : marchés globaux
    marches_globaux = MarcheGlobal.objects.all()

    # 2) Résultats mixtes uniquement si recherche ou filtre actif
    results = []
    if q or type_filter != 'all':
        mg_qs = MarcheGlobal.objects.annotate(obj_type=Value('global', CharField()))
        md_qs = MarcheDetaille.objects.select_related('marche_global').annotate(obj_type=Value('detaille', CharField()))
        c_qs = Carte.objects.select_related('marche_detaille__marche_global', 'solution', 'nom_client').annotate(obj_type=Value('carte', CharField()))

        if q:
            mg_qs = mg_qs.filter(Q(nom__icontains=q) | Q(description__icontains=q))
            md_qs = md_qs.filter(Q(titre__icontains=q) | Q(objet__icontains=q) | Q(description__icontains=q) | Q(matricule__icontains=q))
            c_qs = c_qs.filter(Q(titre__icontains=q) | Q(objet__icontains=q) | Q(contenu__icontains=q) | Q(nom_client__nom__icontains=q))
        ...
        if type_filter == 'global':
            ...
            raw = list(mg_qs) + list(md_qs) + list(c_qs)

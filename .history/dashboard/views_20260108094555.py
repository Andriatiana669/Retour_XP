...
def modifier_marche_view(request, pk):
    marche = get_object_or_404(MarcheDetaille, pk=pk)
    if request.method == 'POST':
        form = MarcheDetailleForm(request.POST, instance=marche)
        if form.is_valid():
            form.save()                       # pas besoin de commit=False ici
            return redirect('dashboard:liste_marche',
                          marche_global_id=marche.marche_global_id)
    else:
        form = MarcheDetailleForm(instance=marche)
    return render(request, 'dashboard/modifier_marche.html',
                  {'form': form, 'marche': marche})

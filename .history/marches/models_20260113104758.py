...
class MarcheDetaille(models.Model):
    ...
    prestation = models.ManyToManyField(Prestation, related_name='marches_prestation', blank=True)
    ...
    def save(self, *args, **kwargs):
        # 1. Sauvegarde temporaire pour obtenir un id
        if not self.pk:
            with transaction.atomic():
                super().save(*args, **kwargs)

        # 2. Calcul du « plus grand index » dans ce marché global
        qs = MarcheDetaille.objects.filter(marche_global=self.marche_global).order_by('id')
        total = qs.count()
        # largeur du numéro (001, 01, 0001…) selon total
        width = len(str(total)) if total > 99 else (3 if total > 9 else 2)

        # 3. on reformate tous les matricules de ce global (décalage auto)
        ...
        # 4. on remet à jour le compteur du global
        self.marche_global.nombre_projet = self.marche_global.marches_detailles.count()
        self.marche_global.save(update_fields=['nombre_projet'])

    def __str__(self):
        return f"{self.nom_marche} - {self.titre or ''}"
...
class Carte(models.Model):
    ...
    nom_client = models.ForeignKey(NomClient, on_delete=models.PROTECT, blank=True, null=True)

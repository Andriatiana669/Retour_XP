...
def export_cartes_xlsx(request, marche_detaille_id):
    ...
    for carte in cartes:
        data.append({
            'Titre': str(carte.titre or ""),
            'Objet': str(carte.objet or ""),
            'Client': str(carte.nom_client or ""),
            'Date retour': carte.date_retour.isoformat() if carte.date_retour else "",
            'Solution': str(carte.solution or ""),
            'Demande démarche': 'Oui' if carte.solution and carte.solution.demande_demarche else 'Non',
            'Démarche solution': str(carte.demarche_solution or ""),
            'Nom dossier': str(carte.nom_dossier or ""),
            'Date livraison': carte.date_livraison.isoformat() if carte.date_livraison else "",
            'Date MAJ': timezone.make_naive(carte.date_maj).isoformat() if carte.date_maj else "",
        })
    ...
    # Utilisation de pandas pour écrire le fichier
    with pd.ExcelWriter(response, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name=f"Cartes {str(marche.nom_marche)[:25]}")

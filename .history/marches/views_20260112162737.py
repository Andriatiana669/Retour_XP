...
def export_cartes_xlsx(request, marche_detaille_id):
    marche = get_object_or_404(MarcheDetaille, id=marche_detaille_id)
    cartes = marche.cartes.all()

    response = HttpResponse(content_type='text/csv; charset=utf-8')
    response['Content-Disposition'] = f'attachment; filename="cartes_{marche.nom_marche}_{datetime.now():%Y%m%d_%H%M}.csv"'
    ...
    for c in cartes:
        writer.writerow([
            c.titre, c.objet, c.nom_client, c.date_retour,
            c.solution, c.solution.demande_demarche,
            c.demarche_solution, c.nom_dossier, c.date_livraison,
            c.date_maj
        ])

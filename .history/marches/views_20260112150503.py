...
def ajouter_carte_view(request, marche_detaille_id):
    ...
    clients_marche = marche.nom_client.all()   # QuerySet

    if request.method == 'POST':
        form = CarteForm(request.POST, request.FILES)
        clients_choisis = request.POST.getlist('clients_choisis')  # liste d’id

        if form.is_valid():
            carte = form.save(commit=False)
            carte.marche_detaille = marche
            carte.save()                          # on a besoin d’une PK pour M2M

            # 1. clients cochés
            clients_ids = request.POST.getlist('clients_choisis')
            if clients_ids:
                carte.nom_client.set(clients_ids)
            else:
                carte.nom_client.clear()

            # 2. champs démarche (si fournis)
            if request.POST.get('demarche_solution'):
                carte.demarche_solution = request.POST['demarche_solution']
            carte.nom_dossier = request.POST.get('nom_dossier', '')
            if request.POST.get('date_livraison'):
                carte.date_livraison = request.POST['date_livraison']
            carte.save()          # sauvegarde finale des extras
...
def download_word_view(request, carte_id):
    ...
    if carte.image and carte.image.file:
        ...
        if os.path.isfile(image_path):
            # largeur 10 cm (ajuste si besoin)

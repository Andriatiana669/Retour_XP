...
def export_cartes_xlsx(request, marche_detaille_id):
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = f"Cartes {str(marche.nom_marche)[:25]}"

    # En-têtes
    headers = [
        'Titre', 'Objet', 'Client', 'Date retour', 'Solution',
        'Demande démarche', 'Démarche solution', 'Nom dossier',
        'Date livraison', 'Date MAJ'
    ]
    ws.append(headers)

    for carte in cartes:
        # Gestion des dates : conversion en chaînes pour éviter les erreurs
        date_retour = carte.date_retour.strftime('%Y-%m-%d') if carte.date_retour else ""
        date_livraison = carte.date_livraison.strftime('%Y-%m-%d') if carte.date_livraison else ""
        date_maj = timezone.make_naive(carte.date_maj).strftime('%Y-%m-%d %H:%M') if carte.date_maj else ""

        ws.append([
            str(carte.titre or ""),
            str(carte.objet or ""),
            str(carte.nom_client or ""),
            date_retour,  # Chaîne formatée
            str(carte.solution or ""),
            'Oui' if carte.solution and carte.solution.demande_demarche else 'Non',
            str(carte.demarche_solution or ""),
            str(carte.nom_dossier or ""),
            date_livraison,  # Chaîne formatée
            date_maj  # Chaîne formatée
        ])

    # Nom du fichier
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    filename = f"cartes_{marche.nom_marche}_{timestamp}.xlsx"

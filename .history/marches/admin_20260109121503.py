...
class MarcheDetailleAdmin(admin.ModelAdmin):
    ...
    # ----------------------------
    # Sections visibles dans l'admin
    # ----------------------------
    fieldsets = (
        (None, {
            'fields': (
        ...
        # La section "Contenu" est supprimée
    )
    ...
    def _badge_list(self, queryset, color):
        ...
        text_color = '#fff' if color == 'primary' else '#000'  # blanc pour responsables, noir sinon

        return format_html(
            '<span style="background:var(--{});color:{};padding:2px 6px;border-radius:8px;font-size:0.7rem;">{}</span>',
            color, text_color, ', '.join(items)
        )

...
class MarcheDetaille(models.Model):
    ...
    def save(self, *args, **kwargs):
        # on sauvegarde d'abord sans toucher aux M2M
        super().save(*args, **kwargs)

        # génération matricule si vide (id existant maintenant)
        if not self.matricule:
            if self.resp_marche.exists():
                resp = self.resp_marche.first()
                self.matricule = f"{resp.matricule}_{self.id}"
            else:
                self.matricule = f"MD_{self.id}"
            super().save(update_fields=['matricule'])

        # mise à jour du compteur du MarcheGlobal

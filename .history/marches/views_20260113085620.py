...
def export_cartes_xlsx(request, marche_detaille_id):
    ...
    # Données – cast exhaustif
    for c in cartes:
        # ─── dates « sans timezone » ───
        livraison = c.date_livraison if c.date_livraison else None   # ← None autorisé
        maj = timezone.make_naive(c.date_maj) if c.date_maj.tzinfo else c.date_maj

        ws.append([
            str(c.titre or ''),
            str(c.objet or ''),
            str(c.nom_client or ''),
            c.date_retour,                                    # native date
            str(c.solution or ''),
            'Oui' if c.solution and c.solution.demande_demarche else 'Non',
            str(c.demarche_solution or ''),
            str(c.nom_dossier or ''),
            livraison,                                        # ← None si pas de date
            maj                                                # naive datetime
        ])
